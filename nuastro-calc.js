...
function calcPlacidus(jd, lat, lng){
  const T=(jd-2451545)/36525;
  const GMST=n360(280.46061837+360.98564736629*(jd-2451545)+0.000387933*T*T);
  const LST=n360(GMST+lng);
  const RAMC=LST;
  const e=(23.439291111-0.013004167*T)*Math.PI/180;
  const latR=lat*Math.PI/180;
  const D2R=Math.PI/180, R2D=180/Math.PI;

  function placidus2(ramcDeg, fraction){
    let lon=n360(ramcDeg+fraction*90);
    for(let i=0;i<50;i++){
      const lonR=lon*D2R;
      const dec=Math.asin(Math.sin(e)*Math.sin(lonR));
      const v=-Math.tan(dec)*Math.tan(latR);
      if(Math.abs(v)>1) break;
      const adv=Math.asin(v)*R2D;
      const sa=90+adv;
      const ra=Math.atan2(Math.sin(lonR)*Math.cos(e),Math.cos(lonR))*R2D;
      const newLon=n360(ramcDeg+(fraction*sa));
      const newLonR=newLon*D2R;
      const newRa=Math.atan2(Math.sin(newLonR)*Math.cos(e),Math.cos(newLonR))*R2D;
      const prev=lon;
      lon=n360(Math.atan2(Math.sin(newRa*D2R)/Math.cos(e),Math.cos(newRa*D2R))*R2D);
      if(Math.abs(lon-prev)<0.001) break;
    }
    return n360(lon);
  }
  ...
  if(mc<0)mc+=360;
  // ASC
  const yA=-Math.cos(RAMC*D2R);
  const xA=Math.sin(e)*Math.tan(latR)+Math.cos(e)*Math.sin(RAMC*D2R);
  let asc=Math.atan2(yA,xA)*R2D;
  if(asc<0)asc+=360;
  if(n360(asc-n360(mc+90))>180) asc=n360(asc+180);

  const h11=placidus2(RAMC,1/3);
  const h12=placidus2(RAMC,2/3);
  const h2 =placidus2(n360(RAMC+180),2/3);
  const h3 =placidus2(n360(RAMC+180),1/3);
...
function calculate(year, month, day, hour, min, utcOffset, lat, lng){
  ...
  const placidus = (lat!==0||lng!==0) ? calcPlacidus(jdCalc, lat, lng) : null;
