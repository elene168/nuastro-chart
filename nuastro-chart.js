...
function draw(svgEl, result, mode){
  ...
  if(mode === 'nuastro'){
    ...
    // Tropical: outer belt always shows 12 equal 30° signs
    // The signs rotate with ASC — Placidus only affects the inner house lines
    for(let i=0; i<12; i++){
      const startSvg = e2svg(i*30);
      const p1=pt(startSvg,Ro), p2=pt(startSvg,Rm);
      svgEl.appendChild(ln(p1.x,p1.y,p2.x,p2.y,'#111110',0.9));
      const gp=pt(e2svg(i*30+15),(Ro+Rm)/2);
      svgEl.appendChild(tx(TROP12[i].abbr,gp.x,gp.y,
        {sz:11,fill:'#111110',font:"'Cormorant Garamond',serif",wt:'600'}));
    }
  } else {
    // Vedic or fallback: 12 equal 30° signs
    for(let i=0; i<12; i++){
      const sigLon = i*30;
  ...
  if(mode === 'nuastro'){
    ...
    // Tropical: Placidus unequal inner houses
    // Vedic: 12 equal 30° inner houses
    if(mode === 'tropical' && result.placidus){
      const cusps = result.placidus;
      for(let i=0; i<12; i++){
        const cuspLon = cusps[i];
        const nextLon = cusps[(i+1)%12];
        // Only draw spoke if this cusp is not within 2° of a sign boundary
        // (avoids doubled lines and near-zero slivers)
        const distToSignBoundary = Math.min(n360(cuspLon) % 30, 30 - (n360(cuspLon) % 30));
        const spokeAng = e2svg(cuspLon);
        const p1=pt(spokeAng,Rm), p2=pt(spokeAng,Rc);
        svgEl.appendChild(ln(p1.x,p1.y,p2.x,p2.y,'#BBBBBB',distToSignBoundary<2?0.3:0.5));
        // House number at midpoint between cusps
        const span = n360(nextLon - cuspLon);
        // Only draw house number if sector is wide enough
        if(span > 5){
          const midLon = n360(cuspLon + span/2);
          const hn=pt(e2svg(midLon),(Rm+Rc)/2);
          svgEl.appendChild(tx(String(i+1),hn.x,hn.y,{sz:10,fill:'#CCCCCC',wt:'300'}));
        }
      }
    } else {
      // Vedic or fallback: 12 equal 30° houses
      for(let i=0; i<12; i++){
        const spokeAng = h2svg(i*30);
        const p1=pt(spokeAng,Rm), p2=pt(spokeAng,Rc);
        svgEl.appendChild(ln(p1.x,p1.y,p2.x,p2.y,'#BBBBBB',0.5));
        const hn=pt(h2svg(i*30+15),(Rm+Rc)/2);
        svgEl.appendChild(tx(String(i+1),hn.x,hn.y,{sz:10,fill:'#CCCCCC',wt:'300'}));
      }
    }
  }
  ...
  if(mode === 'nuastro'){
    // Nuastro: MC/IC as tick marks in the outer sign ring only.
    // A full cross-chart axis would misalign with the fixed inner house ring
    // (which uses its own fixed coordinate system), so we show MC/IC as
    // constellation-band markers instead.
    const mcSvgAng = e2svg(mcForWheel);
    const icSvgAng = n360(mcSvgAng+180);
    const mcOuter = pt(mcSvgAng, Ro);
    const mcInner = pt(mcSvgAng, Rm);
    const icOuter = pt(icSvgAng, Ro);
    const icInner = pt(icSvgAng, Rm);
    svgEl.appendChild(ln(mcOuter.x,mcOuter.y,mcInner.x,mcInner.y,'#111110',1.8));
    svgEl.appendChild(ln(icOuter.x,icOuter.y,icInner.x,icInner.y,'#111110',1.8));
    // Label just outside the outer ring, away from center
    const mcLbl = pt(mcSvgAng, Ro+14);
    const icLbl = pt(icSvgAng, Ro+14);
    svgEl.appendChild(tx('MC',mcLbl.x,mcLbl.y,lS));
    svgEl.appendChild(tx('IC',icLbl.x,icLbl.y,lS));
  } else {
    // Tropical / Vedic: full axis line through center
    const mcSvgAng = e2svg(mcForWheel);
    const icSvgAng = n360(mcSvgAng+180);
    ...
    svgEl.appendChild(tx('MC',topPt.x,Math.max(14,topPt.y-14),lS));
    svgEl.appendChild(tx('IC',botPt.x,Math.min(666,botPt.y+14),lS));
  }
