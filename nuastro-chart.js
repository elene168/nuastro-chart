...
function draw(svgEl, result, mode){
  ...
  if(mode === 'nuastro'){
    ...
    // Tropical: Placidus unequal houses
    // result.placidus = array of 12 cusp longitudes [H1..H12]
    const cusps = result.placidus || (() => {
      // Fallback: whole-sign if placidus not available
      const s = Math.floor(ascForWheel/30)*30;
      return Array.from({length:12},(_,i)=>n360(s+i*30));
    })();
    ...
    for(let i=0; i<12; i++){
      ...
      let mid = n360(cuspLon + n360(nextCusp-cuspLon)/2);
      const hn = pt(e2svg(mid),(Rm+Rc)/2);
      svgEl.appendChild(tx(String(i+1),hn.x,hn.y,{sz:10,fill:'#CCCCCC',wt:'300'}));
    }
  }
