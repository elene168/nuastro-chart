...
function draw(svgEl, result, mode){
  ...
  // ── FOUR AXIS LINES — all through exact center (cx, cy) ─────────────────────
  // ASC–DSC: always perfectly horizontal
  svgEl.appendChild(ln(cx-Ro,cy,cx+Ro,cy,'#111110',1.5));

  // MC–IC: drawn at actual ecliptic MC position
  // Both endpoints computed from center outward — guaranteed through center
  const mcSvgAng = e2svg(mcForWheel);
  const icSvgAng = n360(mcSvgAng + 180); // exactly opposite = through center
  const mcPt = pt(mcSvgAng, Ro);
  const icPt = pt(icSvgAng, Ro);
  svgEl.appendChild(ln(mcPt.x,mcPt.y,icPt.x,icPt.y,'#111110',1.5));

  // Labels — MC at top (smaller y = closer to top of screen), IC at bottom
