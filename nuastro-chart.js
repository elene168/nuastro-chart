...
function draw(svgEl, result, mode){
  ...
  } else if(mode === 'tropical' && result.placidus){
    // Tropical: outer belt divided by Placidus cusps (unequal sizes)
    // Placidus cusps define where each house begins on the zodiac belt
    const cusps = result.placidus;
    for(let i=0; i<12; i++){
      const cuspLon = cusps[i];
      const nextLon = cusps[(i+1)%12];
      // Spoke at cusp position on outer belt
      const startSvg = e2svg(cuspLon);
      const p1=pt(startSvg,Ro), p2=pt(startSvg,Rm);
      svgEl.appendChild(ln(p1.x,p1.y,p2.x,p2.y,'#111110',0.9));
      // Sign label at midpoint between this cusp and next
      const span = n360(nextLon - cuspLon);
      const midLon = n360(cuspLon + span/2);
      const gp=pt(e2svg(midLon),(Ro+Rm)/2);
      // Find which sign this midpoint falls in
      const signIdx = Math.floor(n360(midLon)/30);
      svgEl.appendChild(tx(TROP12[signIdx].abbr,gp.x,gp.y,
  ...
  if(mode === 'nuastro'){
    ...
    // Tropical and Vedic: always 12 equal 30° inner houses
    for(let i=0; i<12; i++){
